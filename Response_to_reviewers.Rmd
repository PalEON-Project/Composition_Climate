...
We have added the sentence: *While logging and altered fire regimes have altered community composition in the north, they have not widely excluded taxa from the northern regions and associated climates, except for * Tsuga * (Figure 3), a shade-tolerant and late-successional taxon.*
...
We have changed the $x$-axis to a $log_{10}$ scale and added more tick marks to the $x$-axis to make it clear that it is non-linear.
...
*The total observed shift (d_tot) between a historical and modern PDF is the Hellinger distance between V_M C_M and V_H C_H (Figure 2a: top left to bottom right). The conditions V_M C_H and V_H C_M are synthetic, generated by superimposing historical climate patterns on modern vegetation distributions and *vice versa*. The effect of climate change on shifting PDFs (d_c) is the difference between PDFs using the early climate normals (C_H) and modern climate normals (C_M) overlaid on PLS vegetation (e.g., V_H C_H versus V_H C_M; Figure 2a; left column). The effect of vegetation change on shifting PDFs is d_v, the Hellinger distance between V_H C_H and V_M C_H (top row of Figure 2a). Because Euro-American land-use is the dominant driver of vegetation change in the upper Midwest over the past two centuries (Introduction), we often attribute shifts recorded by d_v to the land-use change associated with Euro-American settlement.*
...
We have modified the t-tests to include spatial autocorrelation and expanded their description in the Supplementary Information.  The responsible variables are explained more clearly in the methods now.  We compare VHCH and VHCM to estimate the significance and direction of the climate shift (vegetation is fixed, climate layer changes), and compare VHCH to VMCH to estimate the change in vegetation (keeping vegetation constant).
...
We have added more information to the Methods section, while also retaining much of this information in the Figure legends.  We prefer a style that uses longer legends, to facilitate figure interpretation.
