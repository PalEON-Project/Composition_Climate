...
*When long term records (Burnette et al., 2010; Baker et al., 1985) are considered, $\Delta t_{min}$ increases to approximately `r round(clim_change[4], 1) + 0.5`^o^C, $\Delta t_{max}$ to `r round(clim_change[2], 1) + 0.35`^o^C and $\Delta t_{diff}$ to `r round(clim_change[2], 1) - 0.15`^o^C.*
...
I believe that this is addressed in the response to the prior comment.
