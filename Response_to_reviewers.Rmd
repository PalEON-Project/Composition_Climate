...
Interestingly, these shifts do not result in significant changes to the overall climate envelope for the tree species (compare the paper figure with the figure below), indicating the influence of land use is much larger than for climate for most taxa.

![](figures/boxplot_corrected.png)
...
We have included spatial autocorrelation in the t-tests using the `SpatialPack` function`modified.ttest`.  We use confidence intervals based on the raw t.test scores, but report significance based on the modified.ttest calculation.
