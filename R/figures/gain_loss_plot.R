...
get_loss_gain <- function(unit.raster, best.taxa, pls.data, agg.dens){
...
loss_melt <- get_loss_gain(unit.raster, best.taxa, pls.data, agg.dens)

loss.plot <- base.map +
    geom_tile(data = subset(loss_melt, !variable %in% 'ironwood') , aes(x = x, y = y, fill = factor(value))) +
