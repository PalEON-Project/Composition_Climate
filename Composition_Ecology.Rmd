...
```
![figone](figures/figure_one.tiff)
...
**Figure 2.**.  *Gain loss and continuous presence for PLSS and FIA tree taxa. Loss is assessed as cells that contained presence during the PLSS era, and no presence in the FIA data (light blue). Continuous presence is indicated by dark gray, where both FIA and PLSS cells indicate presence.  Few taxa show novel presence (red), although taxa such as poplar, ash and basswood show gains of over 10%.*

**Table 1.** *Gain and loss of the various forest tree types of interest since the pre-settlement era. The first three columns represent percentages of the total area occupied by the taxon since settlement. Modern and Historical values represent 100s of km^2^ occupied by the tree taxon within either the FIA or PLSS data.*
...
**Figure 3.** *Changes in the realized climate space for genera in the upper Midwestern United States. For forest taxa maximum temperatures decline across the region from the PLS era to the modern era.  At the same time minimum temperatures and annual precipitation have both increased.*
...
**Figure 4**. *Blocked method for analysis, with representative normalized densities for climate distributions.  Data presented represent one of four combinations of pre-settlement vegetation (pink fill, left column) or modern vegetation (blue fill, right column) and pre-settlement climate (solid border, top row) or modern climate (dashed border, lower row).  A diagonal shift from the top left to bottom right represents the true change in forest-climate relationships.*
...
**Figure 5**. *For each of the four variables examined we see differences in the relative contributions of land use and climate in driving patterns of change over the settlement era.  The x-axis represents the total shift (diagonal in Figure 4) from PLSS - FIA for both vegetation and climate, while the y-axis represents the difference between $d_{h}(V_{PLSS}C_{PLSS},V_{PLSS}C_{FIA})$ and $d_{h}(V_{FIA}C_{FIA},V_{PLSS}C_{FIA})$. Taxa with negative y-axis values have changes in distributions along the climate gradient that are driven largely by land use, while positive y-axis values can be largely attributed to regional climate change.*
...
```{r tests, warning=FALSE, message=FALSE, echo=FALSE, results='as-is'}
source('R/confounding_table.R')

pandoc.table(conf_table, justify = "left", style = 'simple')
